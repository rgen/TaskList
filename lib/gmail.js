...
export async function importEmailsAsTasks(userId) {
  // Ensure required columns exist using raw connection
  const migrationClient = await db.connect()
  try {
    await migrationClient.query('ALTER TABLE tasks ADD COLUMN IF NOT EXISTS gmail_message_id TEXT')
    await migrationClient.query('ALTER TABLE tasks ADD COLUMN IF NOT EXISTS source TEXT')
  } catch (e) {
  ...
  // Check which message IDs are already imported (use raw connection for reliability)
  const checkClient = await db.connect()
  let existingIds = new Set()
  try {
    const { rows: existingRows } = await checkClient.query(
      'SELECT gmail_message_id FROM tasks WHERE user_id = $1 AND gmail_message_id IS NOT NULL',
      [userId]
    )
    existingIds = new Set(existingRows.map((r) => r.gmail_message_id))
  } finally {
    checkClient.release()
  }
  ...
  for (const msg of messages) {
    ...
    try {
      ...
      let notes = [from ? `From: ${from}` : '', date ? `Date: ${date}` : '', `Original email: ${emailLink}`, '', body].filter(Boolean).join('\n').trim()
      let priority = 'medium'
      let dueDate = null
      let categoryId = null
      ...
      // Remove TaskList label from the email so it goes back to inbox
      try {
