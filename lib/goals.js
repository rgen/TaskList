...
function getWeekNumber(goalStartDate, weekStartDate) {
  ...
  const current = new Date(weekStartDate + 'T12:00:00Z')
...
export async function generateTasksForWeek(goal, weekStartDate, userId) {
  ...
  const endDate = new Date(goal.end_date + 'T12:00:00Z')
  const weekNum = getWeekNumber(goal.start_date, weekStartDate)
